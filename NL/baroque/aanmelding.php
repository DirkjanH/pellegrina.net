...
require_once $_SERVER['DOCUMENT_ROOT'].'/includes/NAW_NL.php'; ?>
            ...
            <table class="formulier" id="instr_kop">
              <tr>
                ...
                <td><b>Ik meld mij aan als instrumentalist.</b> <span class="nadruk">Vul SVP de vragen die hieronder verschijnen in als je klikt op het vinkje rechts.</span></td>
              </tr>
            </table>
            <table class="formulier" id="instr_vragen">
              <tr>
                <td class="linkerkolom">&nbsp;</td>
                <td><p><strong>Ik bespeel het volgende instrument c.q. instrumenten: <span class="nadruk">(a' = 415 Hz; meerdere keuzes zijn mogelijk door de Ctrl-toets ingedrukt te houden)</span></strong></p>
                  <SELECT NAME="instr[]" size="17" multiple>
                    <OPTGROUP LABEL="Blazers">
                    <OPTION VALUE="112" <?php
					if (isset($instr) and stristr($instr, '112') !== false) echo 'selected'; ?>>traverso</OPTION>
                    <OPTION VALUE="116" <?php
					if (isset($instr) and stristr($instr, '116') !== false) echo 'selected'; ?>>blokfluit</OPTION>
                    <OPTION VALUE="122" <?php
					if (isset($instr) and stristr($instr, '122') !== false) echo 'selected'; ?>>barokhobo</OPTION>
                    <OPTION VALUE="141" <?php
					if (isset($instr) and stristr($instr, '141') !== false) echo 'selected'; ?>>barokfagot</OPTION>
                    </OPTGROUP>

					<OPTGROUP LABEL="Strijkers (barokinstrumenten of moderne instrumenten met darmsnaren en barokstok)">
					...
					if (isset($instr) and stristr($instr, '250') !== false) echo 'selected'; ?>>viola da gamba
					[SVP bij 'opmerkingen' aangeven welke instrumenten je bespeelt: diskant, alt/tenor en/of bas]</OPTION>
                    </OPTGROUP>

					<OPTGROUP LABEL="Continuo-instrumenten">
                    <OPTION VALUE="320" <?php
					if (isset($instr) and stristr($instr, '320') !== false) echo 'selected'; ?>>clavecimbel</OPTION>
                    <OPTION VALUE="330" <?php
					if (isset($instr) and stristr($instr, '330') !== false) echo 'selected'; ?>>orgel</OPTION>
                    <OPTION VALUE="340" <?php
					if (isset($instr) and stristr($instr, '340') !== false) echo 'selected'; ?>>theorbe</OPTION>
                    </OPTGROUP>
                    <OPTION VALUE="000" <?php
					if (isset($instr) and stristr($instr, '000') !== false) echo 'selected'; ?>>anders
                    [SVP bij 'opmerkingen' beschrijven]</OPTION>
                  </SELECT></td>
              </tr>
              <tr>
                <td></td>
              ...
              <tr>
                <td></td>
                <td><textarea name="stukken_i" rows="4" cols="70"><?php 
            ...
            <table class="formulier" id="zang_kop">
              <tr>
                ...
                <td><b>Ik meld mij aan als zanger.</b> <span class="nadruk">Vul SVP de vragen die hieronder verschijnen in</span></td>
              </tr>
            </table>
            <table class="formulier" id="zang_vragen">
              ...
              <tr>
                <td></td>
                <td><strong>
                  <textarea name="stukken_z" rows="4" cols="70"><?php
           ...
           <hr>
            <table class="formulier">
			  ...
			  if (isset($_POST['toehoorder']) AND $_POST['toehoorder'] == 500) echo 'checked'; ?> ></td>
            		<td><b>Ik meld me aan als toehoorder</b><span class="nadruk"><b> </b>(prijs <?php echo euro($cursusdata['toehoorder']); ?> incl. een plaats in een tweepersoonskamer en volpension)</span></td>
           		</tr>
     	</table>
<hr>
...
<span class="nadruk">De basisprijs is incl. een plaats in een tweepersoonskamer en volpension. Als je dat wilt hoef je niets aan te vinken.</span></p>
    <table class="formulier">
      ...
      <tr>
		  <td class="linkerkolom">&nbsp;</td><td><input name="diner" type="checkbox" id="diner" value="1"
						<?php if (isset($_POST['diner']) AND $_POST['diner'] == 1) echo 'checked'; ?>><label> Hoewel ik mijn eigen accommodatie regel, maak ik graag ook gebruik van het gezamenlijke diner<span class="nadruk"> (meerprijs <?php echo euro($cursusdata['diner']) ?>)</span></label></td>
      </tr>
    </table>
...
<textarea name="opmerkingen" cols="70" rows="5" ><?php if (isset($_POST['opmerkingen'])) echo $_POST['opmerkingen']; ?></textarea></td>
		...
		<hr>
 			...
 			<table border="0" class="formulier">
			  ...
			  <tr><td><input type="radio" name="betaling" value="transfer"></td><td><label>Ik betaal de aanbetaling per bankoverschrijving. <span class="nadruk">(Gebruik deze optie als het cursusgeld lager is dan <?php echo euro($cursusdata['inschrijfgeld']*2); ?>, bijvoorbeeld voor de Centraal-Europese of Tsjechische prijzen. De aanbetaling is vereist voor verdere verwerking)</span></label></td></tr>
            </table>   
