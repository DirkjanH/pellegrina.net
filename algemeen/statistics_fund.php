...
<p>Fund for Music Students and Eastern European Participants&nbsp;</p>
<table id="stat">
<?php  
$i = $eerstecursus;
while ($i <= $laatstecursus) {
    ...
    <td valign="top">
       <ul>
          <li>Participants: <?php echo $aangenomen[$i]; ?>
             <ul>
                <li>Of whom students: <?php echo $student[$i]; ?></li>
                <li>Of whom Eastern Europeans: <?php echo $oost[$i]; ?></li>
                <li>Of whom Eastern European students: <?php echo $ooststudent[$i]; ?></li>
                <li>Of whom other participants applying for a reduction: <?php echo $reductor[$i]; ?></li>
             </ul>
          </li>
          <li>Donations by participants:&nbsp;<?php echo euro($cursus[$i]['donatie']); ?></li>
          <li>Number of donators:&nbsp;<?php echo $donator[$i].' ('.round($donator[$i]/$aangenomen[$i]*100, 1).' %)'; ?></li>
          <li>Average donation per donator: <?php echo euro($cursus[$i]['donatie']/$donator[$i]); ?></li>
          <li>Average donation per participant: <?php echo euro($cursus[$i]['donatie']/$aangenomen[$i]); ?><br><br>
             <ul>
               <li>Reductions applicants:&nbsp;<?php echo euro($cursus[$i]['korting']); ?></li>
          <?php          if ($ACMP) {?>
               <li>Reductions students:&nbsp;<?php echo euro($cursus[$i]['student']); ?></li>
               <li>Reductions Eastern Europeans:&nbsp;<?php echo euro($cursus[$i]['oost']); ?></li>
               <li>Reductions Eastern European students:&nbsp;<?php echo euro($cursus[$i]['ooststudent']); ?></li>
<?php     $cursus[$i]['korting'] += $cursus[$i]['student'] + $cursus[$i]['oost'] + $cursus[$i]['ooststudent'];
         }?>
            </ul>
          <li>Reductions total:&nbsp;<?php echo euro($cursus[$i]['korting']); ?></li>
          <li>People benefitting from a reduction: <?php echo $reductor[$i] + $reductor2[$i].' ('.round(($reductor[$i] + $reductor2[$i])/$aangenomen[$i]*100, 1).' %)'; ?></li>
          <li>Average reduction:&nbsp;<?php if($reductor[$i] + $reductor2[$i] > 0)  echo euro($cursus[$i]['korting']/($reductor[$i] + $reductor2[$i])); else echo '-' ?></li>
       </ul>       </td>
    <?php 
	$i++;
	}
?>
  ...
  <tr>
    <td colspan="<?php echo $aantal_cursussen; ?>" valign="top"><p>Total participants:&nbsp;<?php echo $aangenomen['totaal']; ?> | donations:&nbsp;<?php echo euro($cursus['totaal']['donatie']); ?> |
        donators: <?php echo $donator['totaal'].' ('.round($donator['totaal']/$aangenomen['totaal']*100, 1).' %)'; ?> | reductions:&nbsp;<?php echo euro($cursus['totaal']['korting']); ?> | people benefitting from a reduction: <?php echo ($reductor['totaal'] + $reductor2['totaal']).' ('.round(($reductor['totaal'] + $reductor2['totaal']) / $aangenomen['totaal']*100, 1).' %)'; ?></p></td>
  </tr>
</table>
<!-- InstanceEndEditable -->
