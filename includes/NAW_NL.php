...
<?php if ((empty($_POST["MM_insert"])) or ($_POST["MM_insert"] != "aanmelding"))
		{
			$opening = "<tr>
               <td>
                  ...
                  <ul>
                     <li>Je kunt je inschrijven voor deze cursus door middel van dit formulier, 
								dat <b>volledig</b> ingevuld dient te worden</li>
                     <li>Je aanmelding wordt verwerkt zodra de <b>aanbetaling van €&nbsp;{$cursusdata['inschrijfgeld']}</b> is
                        ontvangen op rekening NL33 ASNB 0707 2500 72 (BIC ASNB NL21) t.n.v. <em>La Pellegrina</em>, Utrecht</li>
                     <li>Je ontvangt dan een voorlopige bevestiging van inschrijving. Uiterlijk
                        op <b>{$cursusdata['beslisdatum']}</b> wordt de definitieve bezetting bekend gemaakt</li>
                     <li>Door inschrijving verklaar je je akkoord met de voorwaarden zoals vermeld onder 
								<a href=\"praktisch.php\">praktische zaken</a> en elders op de site.</li>
                  </ul></td>
            </tr>
				<tr>
    ...
    <table class="NAW">
      <tr>
        ...
        <td><input name="geslacht" type="radio" value="M" id="geslacht_man" <?php if (isset($_POST['geslacht']) and ($_POST['geslacht'] == "M")) echo 'checked'; ?> />
          ...
          <label for = "geslacht_vrouw">F</label></td>
      </tr>
      <tr>
        ...
        <td><input type="text" class="date" name="geboortedatum"
							value="<?php
if (isset($geboortedatum))echo safestrtotime('d-m-Y',$geboortedatum); else echo '[dd-mm-jjjj]'; ?>" /></td>
      </tr>
      <tr>
        <td>Achternaam:</td>
      ...
      <tr>
        ...
        <td colspan="4"><input name=email type="email" id="email"
          ...
          <span class="nadruk">(verplicht!)</span></td>
      </tr>
    </table>
    <hr>
    ...
    <p>De prijs van het project bedraagt <strong><?php echo euro($cursusdata['prijs_volledig']) ?></strong> per
      persoon; studenten en mensen jonger dan 36 jaar betalen <strong><?php echo euro($cursusdata['prijs_student']) ?></strong>.
      Bij <strong>tijdige inschrijving</strong> (vóór <?php echo $cursusdata['beslisdatum']; ?>) 
      geldt <strong>een korting van <?php echo euro($cursusdata['korting_vroeg']) ?></strong>.</p>
