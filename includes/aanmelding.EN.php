...
d($adresSQL, $ID);
        ...
        if (isset($dlnmr) and $dlnmr != "") {
			...
			$dlnmrSQL = sprintf("UPDATE dlnmr SET voornaam=%s, tussenvoegsels=%s, achternaam=%s,
                ...
                "date"), quote($_POST['geslacht']), quote($_POST['taal']), quote($_POST['telefoon']), quote($_POST['mobiel']), quote($_POST['email']), quote($_POST['dieet']), quote($_POST['publiciteit']), quote($_POST['naam_aanbrenger']), quote($_POST['publiciteit_tx']), isset($_POST['oost']) ? 1 : 0, isset($_POST['student']) ? 1 : 0, quote($_POST['AdresId_FK']), quote($dlnmr));
		}        
		else
        {
            $password = generatePassword(4, true, false, true, false);
			d($_POST['AdresId_FK']);
            $dlnmrSQL = sprintf("INSERT INTO dlnmr (voornaam, tussenvoegsels, achternaam, naam, password,
                ...
                "date"), quote($_POST['geslacht']), quote($_POST['taal']), quote($_POST['telefoon']), quote($_POST['mobiel']), quote($_POST['email']), quote($_POST['dieet']), quote($_POST['publiciteit']), quote($_POST['naam_aanbrenger']), quote($_POST['publiciteit_tx']), isset($_POST['oost']) ? 1 : 0, isset($_POST['student']) ? 1 : 0, $_POST['AdresId_FK']);
		}
...
if ($error) echo $fout;
			...
			$inschrijfSQL = sprintf("INSERT INTO inschrijving (instrumentalist, instrumenten, `instr`, niveau_i, ervaring_i, stukken_i, solozanger, zanger, zangstem, niveau_z, ervaring_z, stukken_z, rol_z, toehoorder, vervoer, info_korting, acc_wens, eenpersoons, kamperen, hotel_2pp, hotel_1pp, eigen_acc, diner, storting_fonds, donatie, opmerkingen, aanbetaling, voorwaarden, DlnmrId_FK, CursusId_FK, datum_inschr) 
	...
	if ($inschrijving_opgeslagen) {
		...
		if (isset($_POST['betaling']) AND $_POST['betaling'] == 'mollie') {		
			try {
				$payment = $mollie->payments->create( [ "amount" => [ "currency" => "EUR", "value" => $inschrijfgeld ],
