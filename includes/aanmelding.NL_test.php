	...
	if ($error) {
		...
		if (isset($dlnmr) and $dlnmr != "")
			$dlnmrSQL = sprintf("UPDATE dlnmr SET voornaam=%s, tussenvoegsels=%s, achternaam=%s,
				...
				$geboortedatum,
				"date"
			), quote($_POST['geslacht']), quote($_POST['taal']), quote($_POST['telefoon']), quote($_POST['mobiel']), quote($_POST['email']), quote($_POST['dieet']), quote($_POST['publiciteit']), quote($_POST['naam_aanbrenger']), quote($_POST['publiciteit_tx']), quote(isset($_POST['oost']) ? "true" : "", "defined", "'1'", "'0'"), quote(isset($_POST['student']) ? "true" : "", "defined", "1", "0"), quote($_POST['AdresId_FK']), quote($dlnmr));

		else {
			$password = generatePassword(4, true, false, true, false);
			$dlnmrSQL = sprintf(
				"INSERT INTO dlnmr_test (voornaam, tussenvoegsels, achternaam, naam, password,
		...
		if ($error) echo $fout;
			...
			$inschrijfSQL = sprintf(
				"INSERT INTO inschrijving_test (instrumentalist, instrumenten, `instr`, niveau_i, ervaring_i, stukken_i, groot_ensemble1, groot_ensemble2, solozanger, zanger, zangstem, niveau_z, niveau_s, ervaring_z, stukken_z, stukken_s, rol_z, danser, niveau_d, ervaring_d, stukken_d, toehoorder, vervoer, info_korting, acc_wens, eenpersoons, kamperen, hotel_2pp, hotel_1pp, eigen_acc, maaltijdpas, storting_fonds, donatie, opmerkingen, aanbetaling, voorwaarden, ACMP, DlnmrId_FK, CursusId_FK, datum_inschr) 
 ...
 */
			...
			$message .= "<p>Je aanmelding wordt verwerkt zodra de betaling van € {$cursusdata['inschrijfgeld']} inschrijfgeld is ontvangen op onze bankrekening <strong>NL60 BUNQ 2177 4957 25</strong> t.n.v. La Pellegrina te Utrecht. Voor betaling van buiten de EU: BIC BUNQ NL2A.</p>";
