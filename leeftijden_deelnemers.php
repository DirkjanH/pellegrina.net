...
require_once ($_SERVER['DOCUMENT_ROOT'].'/includes/includes2014.php');
...
// begin Recordset
$query_Cursus = "SELECT d.naam, (YEAR(CURDATE())-YEAR(geboortedatum)) - (RIGHT(CURDATE(),5)<RIGHT(geboortedatum,5)) AS leeftijd,  CONCAT(a.adres, \", \", a.postcode, \" \", a.plaats, \", \", a.land) as \"adres\", d.telefoon, 		d.mobiel, d.email, i.instr, i.instrumenten, i.toehoorder, i.zangstem, i.vervoer, i.aangenomen FROM dlnmr d, adres a, inschrijving i WHERE i.CursusId_FK = {$CursusId} AND d.adresid_FK = a.adresid AND d.dlnmrid = i.dlnmrid_fk 
...
?>
    <tr <?php if ($Cursus->Fields('aangenomen') != 1) echo 'class="niet_aangenomen"';
			  if (strpos($Cursus->Fields('naam'), "XXX") OR strpos($Cursus->Fields('naam'), "YYY") 
			  	OR strpos($Cursus->Fields('naam'), "ZZZ")) echo 'class="gepostuleerd"'; 
	?>>
       <td class="klein"><?php echo $Cursus->Fields('naam'); ?>&nbsp;</td>
       <td class="klein"><?php echo $Cursus->Fields('leeftijd'); ?>&nbsp;</td>
