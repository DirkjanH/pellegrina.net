...
include_once ($_SERVER['CONTEXT_DOCUMENT_ROOT'].'/includes/includes2019.php');
...
$adresbestand = '/home/pellegrina/domains/pellegrina.net/private/contacts.csv'; // in map 'private'
	...
	foreach($lines as $line) {
		...
		if (((isset($data[30]) AND $data[30] !== '') OR (isset($data[32]) AND $data[32] !== '')) AND (strstr($data[28], 'Geen folders') === false)) {
         ...
         if (strstr($adres['email'], ' ::: ')) $adres['email'] = substr($adres['email'], 0, strpos($adres['email'], ' ::: '));
			if ($data[58] != '') $adres['postcode'] = $data[58]; else $adres['postcode'] = $data[67];
         // echo ('Komt  ::: voor? '.strstr($adres['postcode'], ' ::: ').'; substr = '.substr($adres['postcode'], 0, strpos($adres['postcode'], ' ::: ').'<br>'));
...
if (isset($_SESSION['taal']) AND $_SESSION['taal'] == 1) { // test-adressen
	...
	$adressen[1]['email'] =  "dhorringa@google.com";
...
foreach ($cursussen as $cursus) {
	...
	$begindatum = strftime('%A %e %B', strtotime($cursus['datum_begin']));
	$einddatum = strftime('%A %e %B %Y', strtotime($cursus['datum_eind']));
   ...
   <script src="/vendor/tinymce/tinymce/tinymce.min.js"></script>
     <script>	
      tinymce.init({
        ...
        language: 'nl',
        language_url: '/taal_voor_tinymce/nl.js',  // site absolute URL
...
<button name="cursusNr" type="button"
	...
	onclick="klapdiensten('outlookveld');">andere bestanden</button>
           <span class="nadruk">*) = gebied = xx-yy; xx2-yy2</span><br></td>
...
//	echo 'Kolomlengte: '.$kolomlengte.'<br>';
	...
	while ($key < $aantal) {
		...
		while ($i < $kolomlengte AND $key < $aantal) {
			...
			echo ' >&nbsp;'.$adressen[$key]['naam'].'&nbsp;('.$adressen[$key]['postcode'].')<br>';
			$i++;
			$key++;	
			}
