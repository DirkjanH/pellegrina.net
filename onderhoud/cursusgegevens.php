...
if (isset($_POST["submit"]) AND $_POST["submit"] >= $eerstecursus AND $_POST["submit"] <= $laatstecursus) {
	...
	$prijzenSQL = sprintf("UPDATE cursus SET datum_korting=%s, datum_betaling=%s, login_website=%s, max_dlnmrs=%s, prijs_volledig=%s, 
						  prijs_student=%s, prijs_ce=%s, prijs_ce_student=%s, prijs_cr=%s, prijs_cr_student=%s, korting_vroeg_cr=%s, eenpers=%s, 
						  diner=%s, meerpers=%s, hotel_1pp=%s, hotel_2pp=%s, kamperen=%s, toehoorder=%s, korting_vroeg=%s, korting_eigen_acc=%s,
...
foreach($cursussen as &$cursus) {
	$cursus['begindatum'] = str_replace('  ', ' ', strftime('%e/%m',
        strtotime($cursus['datum_begin'])));
    $cursus['einddatum'] = str_replace('  ', ' ', strftime('%e/%m %Y',
...
?>
          <tr>
            ...
            <td class="w3-center"><input name="submit" type="submit" value="<?php echo $i; ?>"></td>
            <?php
}
?>
        </table>
