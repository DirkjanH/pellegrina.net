...
function query_ensemble() {
	...
	$query_aantal_ensembleleden = sprintf("SELECT count(Id) FROM ensemblelid as e, 
...
if ((isset($_POST["submit_werk"])) && ($_POST["submit_werk"] == "Add work")) {
	$insertSQL = sprintf("INSERT INTO werk (componist, voornaam, titel, opmerking, bezetting) VALUES (%s, %s, %s, %s, %s)",
...
if ((isset($_POST["submit_werk"])) && ($_POST["submit_werk"] == "Update work")) {
	$updateSQL = sprintf("UPDATE werk SET componist=%s, voornaam=%s, titel=%s, bezetting=%s, opmerking=%s WHERE Id=%s",
						   GetSQLValueString(htmlspecialchars($_POST['componist']), "text"),
						   GetSQLValueString(htmlspecialchars($_POST['voornaam']), "text"),
						   GetSQLValueString(htmlspecialchars($_POST['titel']), "text"),
						   GetSQLValueString(htmlspecialchars($_POST['bezetting']), "text"),
...
if (empty($_POST["submit_ensemble"]) and isset($_POST["kies_werk"])/*  AND $_SESSION['WerkId'] >=  0*/) {
	if (isset($_SESSION['EnsembleId']) AND $_SESSION['EnsembleId'] != -1) {
		...
		$insertSQL = sprintf("INSERT INTO ensemble (CursusId_FK, `set`, WerkId, opmerking, link, compleet, definitief) 
...
if ((isset($_POST["submit_ensemble"])) && ($_POST["submit_ensemble"] == "Switch to other set")) {
	...
	$checkSQL = sprintf("SELECT d.naam FROM ensemblelid as e, ensemble, inschrijving as i, dlnmr as d 
...
//	echo 'Aantal dubbele inschrijvingen: '.$check).'<br>';
	...
	if ($check == 0) {
		...
		$updateSQL = sprintf("UPDATE ensemble SET `set`=%s, opmerking=%s WHERE Id=%s",
							   $set,
							   GetSQLValueString(htmlspecialchars($_POST['opmerking_ens']), "text"),
							   GetSQLValueString($_SESSION['EnsembleId'], "int"));
...
if ((isset($_POST["submit_ensemble"])) && ($_POST["submit_ensemble"] == "Update ensemble")) {
	...
	$updateSQL = sprintf("UPDATE ensemble SET opmerking=%s, docent1=%s, docent2=%s, ruimte=%s, link=%s, compleet=%s, definitief=%s 
	WHERE Id=%s",
						   GetSQLValueString(htmlspecialchars($_POST['opmerking_ens']), "text"),
...
if ((isset($_POST["submit_ensemble"])) && ($_POST["submit_ensemble"] == "Erase ensemble")) {
	...
	$ensemble = select_query($query_ensemble);
	$deleteSQL = sprintf("DELETE FROM ensemblelid WHERE EnsembleId=%s",
						   GetSQLValueString($_SESSION['EnsembleId'], "int"));
...
$query_ensembles = sprintf("SELECT ensemble.Id as Id, WerkId, CONCAT_WS(': ',componist,titel) as werk, docent1, docent2, ruimte, link, compleet, definitief
...
// Databasehandelingen _ensembleleden

if ((isset($_POST["ensemblelid_bewerken"])) && ($_POST["ensemblelid_bewerken"] == "toevoegen")) {

	$checkSQL = sprintf("SELECT count(*) as aantal FROM ensemblelid, ensemble WHERE EnsembleId=ensemble.Id AND CursusId_FK=%s 
...
//	echo 'Aantal inschrijvingen: '.$check['aantal'].'<br>';
	...
	if ($check['aantal'] != "" AND $check['aantal'] == 0) {
		...
		$toevoegenSQL = sprintf("INSERT INTO ensemblelid (EnsembleId, InschId, InstrId, Bepaling, Instrumenten) VALUES (%s, %s, %s, %s, %s)",
							   GetSQLValueString($_SESSION['EnsembleId'], "int"),
							   GetSQLValueString($_POST['InschId'], "int"),
...
if ((isset($_POST["ensemblelid_bewerken"])) && ($_POST["ensemblelid_bewerken"] == "update")) {
	if ((isset($_POST["instr_bewerken"])) && ($_POST["instr_bewerken"] != ""))
		$editSQL = sprintf("UPDATE ensemblelid SET InstrId=%s, Bepaling=%s WHERE Id=%s",
		...
		$editSQL = sprintf("UPDATE ensemblelid SET Bepaling=%s WHERE Id=%s",
						   GetSQLValueString(htmlspecialchars($_POST['bepaling_bewerken']), "text"),
						   GetSQLValueString($_POST['LidId'], "int"));
...
if ((isset($_POST["ensemblelid_bewerken"])) && ($_POST["ensemblelid_bewerken"] == "delete")) {
	$deleteSQL = sprintf("DELETE FROM ensemblelid WHERE Id=%s",
						   GetSQLValueString($_POST['LidId'], "int"));
...
$query_Inschr = sprintf("SELECT i.InschId, naam, instr 
...
d($Inschr);
if (is_array($Inschr)) $aantal_mensen = count($Inschr); else $aantal_mensen = 0;
...
if (is_array($werken)) $aantal_werken = count($werken); else $aantal_werken = 0;
// end Recordset {Zoek werken}

// begin Recordset Cursusnamen
$query_cursusnaam = sprintf("SELECT cursusnaam_nl FROM cursus WHERE CursusId=%s",
						   GetSQLValueString(($_SESSION['Cursus'] + $cursus_offset), "int"));
...
$query_docenten = sprintf("SELECT DocId, code FROM docenten, cursusdocenten 
...
$query_bezet = sprintf("SELECT k.Id, COUNT(*) as bezet FROM classrooms as k, ensemble 
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td colspan="3" align="center" class="kop"><form name="cursus_set" id="cursus_set" method="post" action="<?php echo $editFormAction; ?>">
  ...
  <tr>
    <!-- ZOEK NAMEN -->
    <td width="220" valign="top" class="zoeknamen"><form id="vinden" method="post" action="<?php echo $editFormAction; ?>">
        <p align="center">Name:&nbsp;
    ...
    <td valign="top">
      <form name="ensemble" method="post" id="ensemble" action="<?php echo $editFormAction; ?>">
        <table width="100%" border="1" class="selectie">
          ...
          <tr valign="baseline">
            <td align="right" valign="middle"><div align="right">Remarks:</div></td>
        ...
        <table width="100%" border="1" class="selectie">
          ...
          <tr valign="baseline" class="set<?php echo $_SESSION['Set']; ?>">
            ...
            <td valign="baseline"><strong><?php echo stripslashes($ensemble['werk']); ?></strong>&nbsp;complete:
              ...
              &nbsp;definitive: <input name="definitief" type="checkbox" <?php if ($ensemble['definitief'] == '1') echo 'checked'; ?>> nr. of participants: <?php echo $ensemble['aantal'];?></td>
          ...
          <tr valign="top" class="set<?php echo $_SESSION['Set']; ?>">
            <td width="50" align="right" valign="middle"><div align="right">Remark:</div></td>
          ...
          <tr valign="baseline" class="set<?php echo $_SESSION['Set']; ?>">
            <td colspan="3" align="center">&nbsp;
              ...
              &nbsp;
              <input name="submit_ensemble" type="submit" id="submit_ensemble" value="Erase ensemble"></td>
          </tr>
        </table>
	...
	if ($_SESSION['EnsembleId'] != -1) {
			$query_ensembleleden = sprintf("SELECT id as LidId, naam, InstrId, bepaling, e.instrumenten FROM ensemblelid as e, 
			dlnmr as d, inschrijving as i WHERE EnsembleId = %s AND e.InschId = i.InschId AND i.DlnmrId_FK = d.DlnmrId 
		...
		?>
          <tr valign="baseline" class="set<?php echo $_SESSION['Set']; ?>">
			...
			if (count($ins) == 1) {
				...
				if ($instr == $ensemblelid['InstrId']) $sel = ' SELECTED'; else $sel = '';
		  ...
		  echo $ensemblelid['bepaling']; ?>" size="15"></td>
            <td nowrap="nowrap"><img src="../Images/Logos/b_edit.png" alt="edit" width="16" height="16" 
		  onclick="updateLid(<?php echo $LidId; ?>)"> &nbsp; <img src="../Images/Logos/b_drop.png" alt="delete" width="16" height="16" ]
		  onclick="deleteLid(<?php echo $LidId; ?>)"></td>
          </tr>
          <?php } 
	} ?>
    ...
    <td width="220" valign="top" class="zoek_werk"><form action="<?php echo $editFormAction; ?>" method="post" name="zoek_werk">
        <p align="center">Name:&nbsp;
