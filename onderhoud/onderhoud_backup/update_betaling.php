...
if ((isset($_POST["Verwerk"])) && ($_POST["Verwerk"] == "Verwerk betaling") && $_POST['InschId'] != "") {
	...
	if ($_POST['cash'] == 1) {
  	...
  	$updateSQL = sprintf("UPDATE inschrijving SET aanbet_bedrag=%s, rekening_opmerking=%s WHERE InschId=%s",
                       ...
                       GetSQLValueString($_POST['rekening_opmerking'], "text"),
...
if ($totalRows_inschrijving > 1) {
		...
		echo "<form action=\"{$editFormAction}\" method=\"get\" name=\"inschrijving\" id=\"inschrijving\"> \n <select name=\"cursus\" size=\"{$totalRows_inschrijving}\" >";
      	foreach ($inschrijving as $ins){
			...
			if (!(strcmp($ins['CursusId_FK'], $_GET['cursus']))) {echo "SELECTED";}
...
<label>
    ...
    &nbsp;Betaling tijdens cursus in contanten afgesproken:
      ...
      <tr valign="baseline">
        <td colspan="2"><p class="groot">Nog openstaande rekeningen:</p>
          <div id="navcontainer">
		    <ul id="navlist">
			<?php 
