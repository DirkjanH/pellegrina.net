...
$query_inschrijving = sprintf("SELECT
  voornaam,
  naam,
  adres,
  postcode,
...
if (empty($_GET['DlnmrId']) or $_GET['DlnmrId'] == "" or $_POST['leegmaken'] == 'Leegmaken') $id = -1; else $id = $_GET['DlnmrId'];
...
$cursusnaam = $cursus[$_POST['CursusId_FK']]['login_website'];
	...
	if ($_POST['taal'] == "NL") $tsjecho_text = file_get_contents($_SERVER["DOCUMENT_ROOT"].'/bevestiging/bev_'.$cursusnaam.'_NL.txt');
	else $tsjecho_text = file_get_contents($_SERVER["DOCUMENT_ROOT"].'/bevestiging/bev_'.$cursusnaam.'_EN.txt');
...
//	echo 'voor of op actiedatum<br>';
	if ($_POST['taal'] == "NL") $mail_text = file_get_contents($_SERVER["DOCUMENT_ROOT"].'/bevestiging/voorl. bev. NL_voor_1-3.htm');
	else $mail_text = file_get_contents($_SERVER["DOCUMENT_ROOT"].'/bevestiging/voorl. bev. EN_voor_1-3.htm');
}
else {
//	echo 'na actiedatum<br>';
	if ($_POST['taal'] == "NL") $mail_text = file_get_contents($_SERVER["DOCUMENT_ROOT"].'/bevestiging/voorl. bev. NL_na_1-3.htm');	
	else $mail_text = file_get_contents($_SERVER["DOCUMENT_ROOT"].'/bevestiging/voorl. bev. EN_na_1-3.htm');	
}

$mail_text = str_replace("{voornaam}", $_POST['voornaam'], $mail_text);
if ($_POST['taal'] == "NL" OR $_POST['taal'] == 'DE') {
...
<table>
  <tr>
    <td colspan="5"><form id="zoek" name="zoek" method="get" action="<?php echo $editFormAction; ?>">
        Id:
...
if ($aantal_inschrijvingen > 1) {
		...
		echo "<form action=\"{$editFormAction}\" method=\"get\" name=\"inschrijving\" id=\"inschrijving\"> \n <select name=\"cursus\" size=\"{$aantal_inschrijvingen}\" >";
     	foreach($inschrijving as $i => $inschr){
			...
			if (!(strcmp($inschr['CursusId_FK'], $_GET['cursus']))) {echo "SELECTED";}
...
?>
  <tr>
    ...
    <td valign="bottom"><h2><?php echo $dlnmr['naam']; ?></h2></td>
    <td valign="bottom"><h2>
      <div class="rechts"> Taal:</div>
    </h2></td>
    <td valign="bottom"><h2><?php echo $dlnmr['taal']; ?></h2></td>
    <td valign="bottom">&nbsp;</td>
  </tr>
  <form action="<?php echo $editFormAction; ?>" method="POST" name="update" id="update">
    <tr>
      <td height="0" colspan="5"><?php if ($insch['CursusId_FK'] != "") echo "Inschrijving nr. 
    ...
    <tr valign="baseline">
      ...
      <td><input name="aangenomen" type="checkbox" id="aangenomen" value="1" <?php if 
				(!(strcmp($insch['aangenomen'],1))) {echo "checked";} ?> />
        <label>Leeftijd:&nbsp;
      ...
      <td height="30"><input name="afgewezen" type="checkbox" id="afgewezen" <?php 
				if (!(strcmp($insch['afgewezen'],1))) {echo "checked";} ?> /></td>
      <td>&nbsp;</td>
    </tr>
    <tr valign="baseline">
...
?>
      ...
      <td>&#8364;&nbsp;
        <input type="text" name="cursusgeld" value="<?php if (isset($_GET['DlnmrId']) 
				AND $_GET['DlnmrId'] > 0) echo (float)$factuur['cursusgeld']; ?>" size="6" /></td>
      ...
      <td>&#8364;&nbsp;
        ...
        <INPUT TYPE="button" NAME="inschr" VALUE="€ 250" 
				onClick="self.document.update.aanbet_bedrag.value='250'"></td>
      <td>&nbsp;</td>
    </tr>
    <tr valign="baseline">
      <td><div align="right">Voorlopige bevestiging: <?php echo($insch['voorl_bev']); ?></div></td>
      <td><input type="checkbox" name="voorl_bev" <?php if ($insch['voorl_bev'] > 0) {echo "checked"; $voorl_bev = TRUE;} ?> /></td>
    ...
    <tr valign="baseline">
      ...
      <td colspan="4"><textarea name="wensen"><?php 
			if (isset($_GET['DlnmrId']) AND $_GET['DlnmrId'] > 0) echo $factuur['wensen']; ?></textarea></td>
    </tr>
    <tr valign="baseline">
      <td align="right" valign="top" nowrap><div align="right">Donatie </div></td>
      <td colspan="4"><textarea name="donatietxt"><?php 
			if (isset($_GET['DlnmrId']) AND $_GET['DlnmrId'] > 0) echo $factuur['donatie']; ?></textarea></td>
    </tr>
    <tr valign="middle">
