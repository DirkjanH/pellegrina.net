...
function instrument($instr) {
	...
	if (is_array($ins) AND $ins[0] != '') {
		foreach ($ins as $in) if (isset($instrumententabel[$in]) AND $instrumententabel[$in] != '') $instrumenten[] = $instrumententabel[$in]; else echo 'De index = '.$in.'<br>';
...
function lijst($tabel) {
		global $cursus;
	 	if (is_array($tabel)) $aantal = count($tabel); else $aantal = 0;
		if ($aantal > 0) {
...
if (isset($_GET['DlnmrId']) and ($_GET['DlnmrId'] != "") and isset($_POST["verzend"]) and ($_POST['verzend'] == "Update deelname herhalingsconcert")) {
  $updateSQL = sprintf("UPDATE inschrijving SET herhaling=%s, herhaling_txt=%s WHERE DlnmrId_FK=%s AND CursusId_FK = %s",
...
$query_deelnemer = sprintf("SELECT naam, instr, herhaling, herhaling_txt FROM inschrijving, dlnmr WHERE DlnmrId_FK = DlnmrId AND CursusId_FK = %s AND DlnmrId_FK = %s", 
	$cursus,
...
[weet nog niet] | </label>deel aan herhalingsconcert <?php echo $herhaling[$cursus] ?></p>
