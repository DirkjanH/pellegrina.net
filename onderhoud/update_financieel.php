...
if ( empty( $_GET[ 'DlnmrId' ] )or $_GET[ 'DlnmrId' ] == ""
...
if ( ( isset( $_POST[ "bevestig" ] ) ) && ( $_POST[ "bevestig" ] == "Bevestig aanmelding" ) ) {
	$updateSQL = sprintf( "UPDATE inschrijving SET storting_fonds=%s, donatie=%F, aanbetaling=now(), aangebracht=%s, aangenomen=%s, 		
	...
	if ( $_POST[ 'taal' ] == "NL" ) $info_per_cursus = file_get_contents( $_SERVER[ "DOCUMENT_ROOT" ] . '/bevestiging/bev_' . $cursusnaam . '_NL.txt' );
	else $info_per_cursus = file_get_contents( $_SERVER[ "DOCUMENT_ROOT" ] . '/bevestiging/bev_' . $cursusnaam . '_EN.txt' );
...
<SCRIPT TYPE="text/javascript">
	...
	function ConfirmMsg() {
		...
		} else if ( self.document.update.aanbet_bedrag.value < 300 )
		{
...
?>
				...
				AND $_GET['DlnmrId'] > 0) echo (float)$factuur['cursusgeld']; ?>" size="6"/>
				</td>
				...
				echo (float)$insch['aanbet_bedrag']; ?>" size="6"/>
					<INPUT TYPE="button" NAME="inschr" VALUE="€ 300" onClick="self.document.update.aanbet_bedrag.value='300'">
				</td>
				<td>&nbsp;</td>
			</tr>
			<tr valign="baseline">
				<td>
					<div align="right">Voorlopige bevestiging:
						<?php d($insch['voorl_bev']); ?>
					</div>
				</td>
				<td><input type="checkbox" name="voorl_bev" <?php if ($insch['voorl_bev'] != NUll and $insch['voorl_bev'] > 0) {echo "checked"; $voorl_bev = TRUE;} ?> />&nbsp;</td>
			...
			if (isset($_GET['DlnmrId']) AND $_GET['DlnmrId'] > 0) echo $factuur['wensen']; ?></textarea>
				</td>
			</tr>
			<tr valign="baseline">
				<td align="right" valign="top" nowrap>
			...
			if (isset($_GET['DlnmrId']) AND $_GET['DlnmrId'] > 0) echo $factuur['donatie']; ?></textarea>
				</td>
			</tr>
			<tr valign="middle">
				<td align="right" valign="top" nowrap>
