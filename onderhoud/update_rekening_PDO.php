...
d($query_inschrijving);
	...
	echo 'Totaal te versturen rekeningen: ' . count($inschrijving) . '<br>';
	}
...
d($ins);
		...
		if ($totaal > 0) {
			...
			$mail_text = str_replace("{aanbet_bedrag}", number_format(($inschr['aanbet_bedrag']*-1), 2), $mail_text);
			$mail_text = str_replace("{cursusgeld}", number_format($totaal, 2), $mail_text);
			...
			$tebetalen = $totaal - $inschr['aanbet_bedrag'];
			$mail_text = str_replace("{tebetalen}", number_format($tebetalen, 2), $mail_text);
			$mail_text = str_replace("{wensen}", stripslashes($inschr['wensen']), $mail_text);
			if ($factuur[donatie] > 0) 		
			$mail_text = str_replace("{donatie}", number_format($factuur[donatie], 2), $mail_text); 
			...
			if (!($_POST['verzenden'])) {
				...
				$update_inschrijving = sprintf("UPDATE inschrijving SET rekening_verzonden=NOW() WHERE InschId=%s",
...
if (isset($aantal_inschrijvingen) AND $aantal_inschrijvingen > 1) {
        ...
        foreach($inschrijving as $in) {
			...
			if (!(strcmp($in['CursusId_FK'], $_GET['cursus']))) {echo "SELECTED";}
...
?>
   <tr>
      <td colspan="2"><h2>Naam:&nbsp;<?php if ($id != -1) echo $inschr['naam']; ?><br>
      </h2></td>
   </tr>
   <tr>
     <td colspan="2"><form action="<?php echo $editFormAction; ?>" method="POST" name="update" id="update">
       <p>
